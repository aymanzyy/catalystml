...
from autoencoder_insitu_train_last_call import SCALEFACTOR
...
def catalyst_execute(info):
    ...
    #grid_and_vel = np.transpose(np.concatenate((xcoords, ycoords, zcoords, x_vel_values), axis=-1), (1, 0))
    grid_and_vel = np.transpose(np.concatenate((xcoords, ycoords, zcoords, scaled_vel_values), axis=-1), (1, 0))

    autoencoder_insitu_train_last_call.main(grid_and_vel, info.cycle)
