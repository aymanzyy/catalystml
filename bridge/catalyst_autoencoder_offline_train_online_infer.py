...
import numpy as np
import offline_train_online_infer as offline_train 
from offline_train_online_infer import SCALEFACTOR
...
def catalyst_execute(info):
    ...
    vel_values = np.expand_dims(np.array(fields["velocity/values"]), axis=-1)
    
    scaled_vel_values = vel_values*SCALEFACTOR
    ...
    numFluidPtsPyth = len(scaled_vel_values)
    
    grid_and_vel = np.transpose(np.expand_dims(np.concatenate((xcoords, ycoords, zcoords, scaled_vel_values), axis=-1), axis=0), (0,2, 1))
    offline_train.infer(grid_and_vel, info.cycle)
